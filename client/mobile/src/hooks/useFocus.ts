...
export function useFocus(): UseFocusReturn {
  ...
  const confirmExecution = useCallback(async () => {
    if (focusState.status !== 'pending' || !focusState.workflowId) {
      return null;
    }
    ...
    try {
      ...
      let result;
      if (focusState.workflowDef?.type === 'rpc') {
        console.log('[Focus] Executing direct RPC:', focusState.workflowId);
        result = await callRpc<any>(focusState.workflowId!, focusState.currentParams);
      } else {
        console.log('[Focus] Executing via orchestrator:', focusState.workflowId);
        result = await callRpc<any>('orchestrator.run', {
          workflowId: focusState.workflowId,
          input: focusState.currentParams
        });
      }
