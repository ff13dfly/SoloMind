...
module.exports = (redis, { serviceName, routerUrl }) => ({
    ...
    async create({ key, type, scope, desc, items }) {
        if (!key) throw Errors.MISSING_PARAM('key');
        key = key.toUpperCase();
        ...
        try {
            ...
            if (reservation.error) {
                throw Errors.INTERNAL_ERROR(reservation.error.message || 'ROUTER_RESERVATION_FAILED');
            }
        } catch (e) {
    ...
    async get({ key }) {
        ...
        if (!data) throw Errors.NOT_FOUND('Category');
    ...
    async update({ key, desc, type }) {
        ...
        if (!existing) throw Errors.NOT_FOUND('Category');
        ...
        try {
            ...
            if (result.error) {
                if (result.error.code === -32012) {
                    throw Errors.INTERNAL_ERROR('ROUTER_PERMISSION_DENIED');
                }
    ...
    async itemAdd({ key, id, label, desc, parentId }) {
        ...
        if (data.items.find(i => i.id === itemId)) {
            throw Errors.ALREADY_EXISTS('Category item');
        }
    ...
    async itemUpdate({ key, id, label, desc, parentId }) {
        if (!key || !id) throw Errors.INVALID_PARAM('key and id required');
        key = key.toUpperCase();
        ...
        if (!item) throw Errors.NOT_FOUND('Category item');
    ...
    async itemRemove({ key, id }) {
        ...
        if (index === -1) throw Errors.NOT_FOUND('Category item');
